...
export const InvoiceDetails: React.FC<{ invoice: Invoice }> = ({ invoice }) => {
  return (
    <div className="bg-white p-6 rounded-lg flex flex-col gap-8 shadow-md">
      <div className="flex flex-col gap-7 justify-start md:flex-row md:justify-between">
        <div>
          <h1 className="text-heading-s text-dark-navy">
            <span className="text-light-slate-blue">#</span>
            {invoice.id}
          </h1>
          <p className="text-body-variant text-light-slate-blue font-[600]">
            {invoice.description}
          </p>
        </div>
        ...
        <div className="flex gap-16 md:gap-36">
          <div className="flex flex-col gap-8">
            <DetailsInfo label="Invoice Date">
              <p className="text-heading-s text-dark-navy">
...
const InvoiceItems: React.FC<{ items: InvoiceItem[]; totalPrice: number }> = ({
  ...
  return (
    <div>
      <div className="bg-[#F9FAFE] p-6 rounded-t-lg flex flex-col gap-4">
        {items.map((item: InvoiceItem) => {
          return (
            <div key={item.name} className="flex justify-between items-center">
              <div className="flex flex-col gap-1">
                <p className="text-dark-navy text-body font-[600]">
                  {item.name}
                </p>
              ...
              <p className="text-body-variant text-dark-navy font-[600]">
      ...
      <div className="bg-shadow-navy px-6 py-8 flex justify-between rounded-b-lg">
        <p className="text-white text-body-variant">Grand Total</p>
        <h1 className="text-heading-m text-white">
